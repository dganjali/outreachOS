...
- [ ] **Secrets → Secret Manager:** `ANTHROPIC_API_KEY`, `MONGODB_URI`, `VOYAGE_API_KEY`, `ENCRYPTION_KEY` (`openssl rand -base64 32`), `CRON_SECRET` (`openssl rand -hex 32`), `GOOGLE_CLIENT_ID/SECRET`, `FIREBASE_SERVICE_ACCOUNT_JSON`, optional `APOLLO_API_KEY`, `SERPER_API_KEY`, `EMAILFINDER_API_KEY`.
...
- [ ] **Contact-pipeline follow-ups** (Serper + emailfinder.dev landed this branch — discovery + SMTP-verified resolution, no more shipped email guesses):
  - [ ] Add a dedicated verifier gate (MillionVerifier / DeBounce — prepaid, never-expire) after resolution, and downgrade catch-all domains to `emailStatus: 'likely'`.
  - [ ] Migrate domain resolution (`resolveCompanyDomain`) off LLM web_search onto Serper too.
  - [ ] Optional `emailResolver` provenance field on `ContactDoc` for analytics (which rung resolved each email).
