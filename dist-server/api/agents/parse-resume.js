...
export default async function handler(req, res) {
    ...
    if (!(await checkRateLimit(scope, res)))
        return;
    const asset = await scope.collection('profile_assets').findById(asset_id);
    if (!asset)
    ...
    try {
        ...
        try {
            ...
            await failRun(scope, run._id, msg);
            await scope.collection('profile_assets').updateById(asset._id, {
                parseError: msg,
                parsedAt: new Date(),
            });
...
function cleanParsed(p) {
    const trimStr = (s, cap = 2000) => typeof s === 'string' ? s.trim().slice(0, cap) : '';
    return {
