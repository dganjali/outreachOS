...
import { apolloEnabled, matchPerson, fullName } from '../_lib/apollo';
export default async function handler(req, res) {
    ...
    try {
        ...
        await scope.collection('profiles').updateById(profile._id, updates);
        const updated = await scope.collection('profiles').findById(profile._id);
...
function pickLinkedinUrl(linkedinUrl, resumeUrl) {
    if (linkedinUrl && /linkedin\.com/i.test(linkedinUrl))
        return linkedinUrl;
    if (resumeUrl && /linkedin\.com/i.test(resumeUrl))
        return resumeUrl;
    return linkedinUrl;
}
function compactApollo(p) {
    return {
        ...
        organization: p.organization
            ...
            : null,
        employment_history: p.employment_history?.slice(0, 8).map((h) => ({
            title: h.title ?? null,
