...
import { CONTACTS_SYSTEM, CONTACTS_RANK_SYSTEM } from '../_lib/prompts';
import { startRun, completeRun, failRun, checkRateLimit } from '../_lib/runs';
import { apolloEnabled, searchPeople, fullName, normalizeEmailStatus, } from '../_lib/apollo';
const TITLE_HINTS_BY_MODE = {
...
export default async function handler(req, res) {
    ...
    const target = await scope.collection('targets').findById(target_id);
    if (!target)
...
async function runWebSearchOnly(args) {
    ...
    return parsed.data.contacts.slice(0, 6).map((c) => ({
        ...
        role: c.role,
        email: c.email ?? null,
        emailStatus: (c.email ? 'guessed' : 'none'),
