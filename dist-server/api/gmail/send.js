import { requireUser, methodNotAllowed } from '../_lib/auth';
import { forUser, newId } from '../_lib/db';
import { getActiveAccessToken, createDraft, sendNow } from '../_lib/gmail';
export default async function handler(req, res) {
    if (req.method !== 'POST')
        return methodNotAllowed(res, ['POST']);
    const user = await requireUser(req, res);
    if (!user)
        return;
    const scope = forUser(user.id);
    const body = (req.body ?? {});
    const sequenceId = body.sequence_id;
    const touchIndex = body.touch_index ?? 0;
    const mode = body.mode ?? 'draft';
    if (!sequenceId)
        return res.status(400).json({ error: 'missing_sequence_id' });
    const seq = await scope
        .collection('email_sequences')
        .findById(sequenceId);
    if (!seq)
        return res.status(404).json({ error: 'sequence_not_found' });
    const contact = await scope.collection('contacts').findById(seq.contactId);
    if (!contact)
        return res.status(404).json({ error: 'contact_not_found' });
    const toEmail = body.to_override?.trim() || contact.email?.trim();
    if (!toEmail) {
        return res.status(400).json({ error: 'no_recipient_email', message: 'Contact has no email. Provide to_override.' });
    }
    // Pick the touch
    let subject;
    let bodyText;
    let priorMessageId = null;
    let threadId = null;
    if (touchIndex === 0) {
        subject = seq.subject;
        bodyText = seq.body;
    }
    else {
        const fu = seq.followups[touchIndex - 1];
        if (!fu)
            return res.status(400).json({ error: 'invalid_touch_index' });
        subject = fu.subject;
        bodyText = fu.body;
        const prior = await scope
            .collection('sent_messages')
            .findOne({ sequenceId, touchIndex: touchIndex - 1 });
        if (prior?.gmailMessageId)
            priorMessageId = `<${prior.gmailMessageId}>`;
        threadId = prior?.gmailThreadId ?? null;
    }
    const tok = await getActiveAccessToken(user.id);
    if (!tok)
        return res.status(412).json({ error: 'gmail_not_connected', message: 'Connect Gmail in Settings first.' });
    const profile = await scope.collection('profiles').findOne();
    // Idempotency on (sequence_id, touch_index)
    const existing = await scope
        .collection('sent_messages')
        .findOne({ sequenceId, touchIndex });
    if (existing && existing.status === 'sent') {
        return res.status(409).json({ error: 'already_sent', sent_message_id: existing._id });
    }
    // Carry profile_version + per-touch refs from the source sequence.
    const refsByTouch = seq.profileRefs ?? {};
    const touchKey = touchIndex === 0 ? 'initial' : `followup_${touchIndex - 1}`;
    const touchRefs = Array.isArray(refsByTouch[touchKey]) ? refsByTouch[touchKey] : [];
    const sentRowBase = {
        sequenceId,
        contactId: contact._id,
        missionId: seq.missionId,
        touchIndex,
        subject,
        body: bodyText,
        toEmail,
        gmailDraftId: null,
        gmailMessageId: null,
        gmailThreadId: null,
        status: 'queued',
        scheduledSendAt: null,
        sentAt: null,
        failedReason: null,
        profileVersionId: seq.profileVersionId ?? null,
        profileRefs: touchRefs,
    };
    let sentRowId;
    if (existing) {
        await scope.collection('sent_messages').updateById(existing._id, sentRowBase);
        sentRowId = existing._id;
    }
    else {
        const created = await scope
            .collection('sent_messages')
            .insertOne({ ...sentRowBase, _id: newId() });
        sentRowId = created._id;
    }
    try {
        const sendArgs = {
            accessToken: tok.accessToken,
            fromEmail: tok.email ?? user.email ?? '',
            fromName: profile?.name ?? undefined,
            toEmail,
            subject,
            body: bodyText,
            threadId: threadId ?? undefined,
            inReplyTo: priorMessageId ?? undefined,
        };
        let result;
        if (mode === 'send') {
            result = await sendNow(sendArgs);
        }
        else {
            result = await createDraft(sendArgs);
        }
        await scope.collection('sent_messages').updateById(sentRowId, {
            gmailDraftId: mode === 'draft' ? result.draftId ?? null : null,
            gmailMessageId: result.messageId,
            gmailThreadId: result.threadId,
            status: mode === 'send' ? 'sent' : 'draft',
            sentAt: mode === 'send' ? new Date() : null,
        });
        if (mode === 'send' && touchIndex === 0) {
            await scope.collection('email_sequences').updateById(sequenceId, {
                status: 'sent',
                sentAt: new Date(),
            });
            await scope.collection('contacts').updateById(contact._id, { status: 'contacted' });
        }
        return res.status(200).json({
            sent_message_id: sentRowId,
            mode,
            gmail_message_id: result.messageId,
            gmail_thread_id: result.threadId,
            gmail_draft_id: mode === 'draft' ? result.draftId : undefined,
        });
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : 'send_failed';
        await scope.collection('sent_messages').updateById(sentRowId, {
            status: 'failed',
            failedReason: msg,
        });
        return res.status(500).json({ error: 'send_failed', detail: msg });
    }
}
